...
import { ElevationGetSchema, GeocodeReverseSchema, GeocodeSearchSchema, GeolocationEstimateSchema, IpGeolocateSchema, NearbyFindSchema, PlacesAutocompleteSchema, PlacesDetailsSchema, PlacesNearbySchema, PlacesPhotosSchema, PlacesSearchTextSchema, RoadsNearestSchema, RoutesComputeSchema, RoutesMatrixSchema, TimezoneGetSchema } from './types.js';
class GoogleMapsMCPServer {
    ...
    async handleGeocodeReverse(args) {
    ...
    async handlePlacesSearchText(args) {
    ...
    async handlePlacesAutocomplete(args) {
    ...
    async handlePlacesDetails(args) {
    ...
    async handlePlacesPhotos(args) {
        const input = PlacesPhotosSchema.parse(args);
        const url = await this.googleMapsClient.placesPhotos(input.photo_reference, input.max_width, input.max_height);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleRoutesCompute(args) {
    ...
    async handleElevationGet(args) {
        const input = ElevationGetSchema.parse(args);
        const results = await this.googleMapsClient.elevationGet(input.locations, input.path, input.samples);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleTimezoneGet(args) {
        const input = TimezoneGetSchema.parse(args);
        const result = await this.googleMapsClient.timezoneGet(input.lat, input.lng, input.timestamp, input.language);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleGeolocationEstimate(args) {
    ...
    async handleIpGeolocate(args) {
        ...
        if (input.reverse_geocode) {
            const geocodeResults = await this.googleMapsClient.geocodeReverse(locationResult.location.lat, locationResult.location.lng, input.language);
            if (geocodeResults.length > 0) {
                normalizedAddress = {
